...
public:
    ...
    void render(const Hittable<T> &world)
    {
        ...
        for (int i = 0; i < m_imageHeight; ++i)
        {
            ...
            {
                const auto pixelCenter = m_pixel00Center +
                                         i * m_pixelDeltaVertical +
                                         j * m_pixelDeltaHorizontal;

                const auto rayDirection = pixelCenter - m_center;
                const Ray<T> ray(m_center, rayDirection);
                const auto pixelColor = rayColor(ray, world);
